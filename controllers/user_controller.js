...
const { successResponse, errorResponse, conflictResponse, invalidResponse, createResponse } = require("../services/response");
  ...
  static async register(req, res) {
    try {
      ...
      if (checkUsername) {
        res
          .status(409)
          .json(conflictResponse("Username sudah digunakan"));
      } else if (checkEmail) {
        res
          .status(409)
          .json(conflictResponse("E-mail number sudah digunakan"));
      } else if (checkPhoneNumber) {
        res
          .status(409)
          .json(conflictResponse("Phone number sudah digunakan"));
      } else {
        ...
        if (isnum) {
          ...
          res
            .status(201)
            .json(createResponse("Registrasi Berhasil", data));
        } else {
